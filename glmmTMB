  ...
  for (int i=0; i < yobs.size(); i++) PARALLEL_REGION {
    if ( !glmmtmb::isNA(yobs(i)) ) {
      ...
      case poisson_family:
        ...
        break;
case bell_family: {
    Type mu_i = mu(i); // Extract mean parameter
    Type theta = lambertW0(mu_i); // Lambert W function for parameterization
    Type comp1 = yobs(i) * log(theta);
    Type comp2 = -exp(theta);
    tmp_loglik = comp1 + comp2;

    SIMULATE {
       yobs(i) = rbell(theta); // Implemented a random generator for Bell distribution in c++
    }
    break;
}
